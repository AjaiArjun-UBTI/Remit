...
import {useSession} from "../components/tenantContext"
...
function EditModal({ claim, onClose, viewOnly = false }: { claim: any; onClose: () => void; viewOnly?: boolean }) {
  ...
  useEffect(() => {
    async function getclaims() {
      const response = await fetch('http://localhost:5050/claims/');
      if(!response.ok) {
  ...
  async function deleteClaim(id) {
    await fetch(`http://localhost:5050/claims/${id}`, {
      method: "DELETE",
    });
    const newClaims = claims.filter((el) => el._id != id);
    setClaims(newClaims);
  } 
  ...
  function claimList() {
    return claims.map((claim) => {
      return (
        <Claim 
        claim={claim}
        deleteClaim = {() => deleteClaim(claim._id)}
        key={claim._id} />
      )
    } )
  }
    ...
    useEffect(() => {
        const fetchUserRole = async () => {
            try {
                ...
                const response = await client.getApplicationRoleService()
                //Extract role from response - adjust based on your API structure
                ...
                setUserRole("admin"); // Fallback to user role
            }
        };
        fetchUserRole();
    }, []);
